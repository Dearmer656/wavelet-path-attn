#!/bin/bash
# PAT-244: K1 L512 scale sweep under current code + fixed optimal cfg convention.
# Per standing rule (2026-08-05): ALL pre-optimal-cfg results (including the old
# S1_me24_s42 sweep, F1=0.7352 at rho=4096) are considered unreliable/buggy and
# must not be cited as baselines. Only K1_L512_optimal_me24_rho4096 (job 551971,
# F1@L2048=0.7200, current code) is a valid K1 data point so far -- everything
# else in this sweep is new.
#
# Scales chosen to map the "effective window" hypothesis (genuine Ricker
# curvature requires rho within roughly [0.25L, 1.0L]; rho >> L collapses to a
# near-linear ramp, confirmed via K3_L512_optimal_center shape analysis) against
# the existing me24/rho4096 (8x L, outside-window) data point:
#   me14 -> rho=128  (0.25x L)
#   me16 -> rho=256  (0.5x L)
#   me18 -> rho=512  (1.0x L)
# with_null (K=1 correct reduction, NOT with_null_independent_scales) +
# wavelet_router_norm_mode=rms_joint + multiscale_norm=rms (inert at K=1).
set -euo pipefail
...
# args: tag  sme  gres_directive
emit_and_submit() {
  local tag="$1" sme="$2" gres="$3"
  local K=1
