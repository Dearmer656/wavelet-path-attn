...
emit_and_submit() {
  local tag="$1" K="$2" sme="$3" norm_mode="$4" gres="$5" multiscale_norm="${6:-none}" router_sigmoid_mode="${7:-with_null}" cosine="${8:-0}"
...
# PAT-244 unified cosine router (wavelet_router_cosine=true) — well-posed learnable-temp design.
# args: tag K sme norm_mode gres multiscale_norm router_sigmoid_mode cosine
emit_and_submit K1_cos_dualtemp    1 14         dual_temp            '#SBATCH --gres=gpu:6000:2'  none with_null                    true
emit_and_submit K1_cos_none        1 14         none                 '#SBATCH --gres=gpu:6000:2'  none with_null                    true
emit_and_submit K3_cos_dualtemp    3 "${K3_SME}" dual_temp           '#SBATCH --gres=gpu:6000:2'  rms  with_null_independent_scales   true
emit_and_submit K3_cos_scalenone   3 "${K3_SME}" dual_temp_scale_none '#SBATCH --gres=gpu:a6000:2' rms  with_null_independent_scales   true
emit_and_submit K3_cos_none        3 "${K3_SME}" none                 '#SBATCH --gres=gpu:6000:2'  rms  with_null_independent_scales   true
echo "=== 5 cosine runs submitted ==="
