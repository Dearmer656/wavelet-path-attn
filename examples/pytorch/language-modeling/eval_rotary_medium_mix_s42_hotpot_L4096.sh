#!/bin/bash
#SBATCH --job-name=rotmed_hp_L4096
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_rotary_medium_s42_L4096.txt
...
# checkpoint at L4096, matching Table 4's small-model methodology 1:1 (front-placed
...
OUTPUT="${BASE}/hotpot_long/results/rotary_medium_s42_ckpt15000/L4096"
mkdir -p "${OUTPUT}"
cd "${BASE}"

MASTER_PORT=$(( 13000 + SLURM_JOB_ID % 10000 ))

echo "=== Rotary medium (finetuned) s42 HotpotQA-Long L4096 ==="
...
python3 -c "import json; d=json.load(open('${OUTPUT}/eval_results.json')); print(f'Rotary medium (finetuned) s42 L4096: F1={d[\"eval_f1\"]:.4f}')"

echo "=== Done: Rotary medium (finetuned) s42 HotpotQA-Long L4096 ==="
