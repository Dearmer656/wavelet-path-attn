#!/bin/bash
#SBATCH --job-name=pat254_task2
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_pat254_task2.txt
...
# PAT-254 Task 2: parameter-group transplant, all 5 groups, both directions,
# K1 rho128 small s42, L512 (where section 3 found delta_train most negative
# -- localizing where THAT deficit comes from is the priority read here).
...
_slack() {
    python3 /project/nlp-work5/hongyu-s/gate1/scripts/notify_slack.py \
        ...
        --issue "PAT-254" \
        --gpu "p6000x1" \
        --summary "PAT-254 Task 2: parameter transplant sweep, K1 rho128 s42, L512"
}
trap '_slack $?' EXIT
...
python analysis/qwab_mechanism_top10/task_02_parameter_transplant/task2_param_transplant.py \
  ...
  --model_tag small_K1rho128_s42 --seq_len 512 --n_case 30 --groups H,QKVO,MLP,LN,embed

echo "=== DONE: PAT-254 Task 2 sweep L512 ==="
