#!/bin/bash
#SBATCH --job-name=pat254_task2_lp
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_pat254_task2_lp.txt
...
# PAT-254 Task 2 follow-up: cumulative layer-prefix swap for QKVO and MLP
# (the two groups that passed the 30%-of-gap threshold in both directions),
# K1 rho128 small s42, L512.
...
_slack() {
    python3 /project/nlp-work5/hongyu-s/gate1/scripts/notify_slack.py \
        ...
        --issue "PAT-254" \
        --gpu "p6000x1" \
        --summary "PAT-254 Task 2 layer-prefix: QKVO+MLP, K1 rho128 s42, L512"
}
trap '_slack $?' EXIT
...
python analysis/qwab_mechanism_top10/task_02_parameter_transplant/task2_param_transplant.py \
  ...
  --model_tag small_K1rho128_s42_layerprefix --seq_len 512 --n_case 30 \
  --groups QKVO,MLP --layer_maxes 3,6,9

echo "=== DONE: PAT-254 Task 2 layer-prefix sweep ==="
