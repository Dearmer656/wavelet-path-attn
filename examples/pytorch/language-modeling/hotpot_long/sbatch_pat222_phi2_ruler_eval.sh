#!/bin/bash
#SBATCH --job-name=pat222_phi2_ruler
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_pat222_phi2_ruler.txt
...
# PAT-222: RULER evaluation on Phi-2 (baseline + motif).
...
_slack() {
    python3 /project/nlp-work5/hongyu-s/gate1/scripts/notify_slack.py \
        ...
        --summary "PAT-222 RULER eval Phi-2 L=${LENGTHS}" 2>/dev/null || true
}
trap '_slack $?' EXIT
set -euxo pipefail
...
OUT=${BASE}/hotpot_long/analysis_outputs/pat222/phi2/ruler
...
echo "=== Done: PAT-222 RULER eval Phi-2 ==="
