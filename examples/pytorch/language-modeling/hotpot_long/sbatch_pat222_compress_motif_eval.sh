#!/bin/bash
#SBATCH --job-name=pat222_compress_motif
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_pat222_compress_motif.txt
...
# PAT-222: compress_motif eval — Self-Extend (all dims) + motif bias indexed by compressed δ'.
# Distill first with sbatch_pat222_llama2_distill_alldims.sh to get llm_motif_L4096_alldims.npz.
...
LAM="${LAM:-1.0}"
N_CASES="${N_CASES:-0}"
# gate_delta: pairs with δ > this use compress_motif; in-range pairs keep true RoPE
GATE_DELTA="${GATE_DELTA:-2048}"
# alpha: compression slope; δ' = GATE + α*(δ − GATE)
ALPHA="${ALPHA:-0.8}"
JSONL_PREFIX="${JSONL_PREFIX:-ruler_v2}"
OUT_TAG="${OUT_TAG:-compress_motif}"
...
_slack() {
    python3 /project/nlp-work5/hongyu-s/gate1/scripts/notify_slack.py \
        ...
        --summary "PAT-222 compress_motif eval L=${LENGTHS} gate=${GATE_DELTA} α=${ALPHA}" 2>/dev/null || true
}
trap '_slack $?' EXIT
set -euxo pipefail
...
echo "=== Done: PAT-222 compress_motif eval L=${LENGTHS} ==="
