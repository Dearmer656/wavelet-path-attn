#!/bin/bash
#SBATCH --job-name=xsum_rotary_med
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/mix_medium_owt_rotary_10ep_s42_fp32/ckpt_eval_xsum/%j_xsum_rotary_medium_mix_10ep.txt
...
# Filtered XSum eval for plain-Rotary GPT-2 medium, mix 10ep s42 checkpoint (mirrors
# eval_filter_xsum_ckpt15900_WR.sh's block_size/batch_size grid exactly for direct
# comparability with QWAB/ALiBi/YaRN: (512,1024,1536) with batch (16,8,4)).
# pe_method=rotary, attn_implementation=eager, no YaRN (this is the plain-RoPE baseline,
# not the YaRN-finetuned checkpoint -- that one already has its own filtered XSum eval,
# eval_xsum_rotary_yarn_finetuned2048_medium_mix.sh).
# checkpoint-15000: same mix-finetune checkpoint used by
# eval_rotary_medium_mix_s42_hotpot_alllen.sh.
...
RESULT_DIR="${WORKDIR}/runs/mix_medium_owt_rotary_10ep_s42_fp32/ckpt_eval_xsum"
RUN_TAG="${SLURM_JOB_ID:-manual}_$(date -u +%Y%m%dT%H%M%SZ)"
OUT_ROOT="${RESULT_DIR}/xsum_out_${RUN_TAG}"
XSUM_CSV="${RESULT_DIR}/xsum_filter_metrics_rotary_medium_mix_10ep_${RUN_TAG}.csv"
...
JOB_PORT=$(( 20100 + ${SLURM_JOB_ID:-0} % 1000 ))
...
echo "=== Rotary medium mix 10ep s42 XSum eval | ckpt: ${CKPT} ==="
...
for i in "${!BLOCK_SIZES[@]}"; do
  ...
  echo "15000,${CKPT},${bs},${batch},${r1},${r2},${rL},${bs_},${cnt},${loss},${ts}" >> "${XSUM_CSV}"
