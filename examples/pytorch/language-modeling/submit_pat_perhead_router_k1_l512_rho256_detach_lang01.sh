#!/bin/bash
# Per-head QWAB scale-weight router, small model, K1, L512, rho=256,
# detach=true. New rho variant alongside the rho=512 per-head+detach line
# (job 580348/580322, F1=0.7181 vs head-shared 0.7041 at ckpt5000, L2048).
# rho=256 => scale_max_exp=16.0 (scale = 2^(max_exp/2) * SCALE_MULTIPLIER_DICT
# ["wavelet"]=1.0, same convention verified against modeling_gpt2.py
# QWABBias.__init__ used throughout this session).
#
# GPU: lang01 (8x a100) per explicit request -- requires
# partition=lang_gpu_long + account=lang (see project CLAUDE.md). bs=8/accum=1
# across 8 GPUs keeps global_bs=64, matching every other run in this
# comparison line.
