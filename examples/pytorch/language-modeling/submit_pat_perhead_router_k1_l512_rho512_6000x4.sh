#!/bin/bash
# Parallel copy of submit_pat_perhead_router_k1_l512_rho512.sh (per-head QWAB
# scale-weight router, small model, K1, L512, rho=512, joint/nodetach), queued
# for 4x6000 (no nodelist pin -- whichever of elm71/72/73 frees up first)
# while the primary copy runs now on elm54 (3090x4). Same experiment, same
# seed=42 -- NOT a second data point, just a race for whichever GPU frees up
# first; decide whether to keep/cancel one once this one actually starts.
# See submit_pat_perhead_router_k1_l512_rho512.sh for the full design/fix
# history (per-head router feature, joint-vs-detach finding, the step-500
# index-out-of-bounds crash and its fix).

set -euo pipefail

WORKDIR="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling"
TAG="perheadrouter_K1_L512_me18_rho512_nodetach_6000x4"
