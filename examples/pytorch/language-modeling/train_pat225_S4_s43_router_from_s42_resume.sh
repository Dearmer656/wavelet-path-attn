#! /bin/bash
#SBATCH --job-name=PAT225_s43_routerS42
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/pat225_scale_card/S4_s43_router_from_s42/train/%j_pat225_s43_router_from_s42.txt
#SBATCH --partition=gpu_long
#SBATCH --gres=gpu:6000:2 --nodelist=elm73
#SBATCH --time=100:00:00
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4

# PAT-225 anchor/transplant causal experiment: same recipe/seed as S4_s43
# (own data order, dropout, backbone init) but the wavelet_ctx_router weights
# are transplanted from S4_s42's CONVERGED checkpoint-15000 (its final,
# well-performing router policy) and FROZEN for the entire run.
#
# Question: does forcing s43's training to use s42's learned router policy
# reproduce s42-like L4096 extrapolation (~0.70), or does it stay near s43's
# natural outcome (~0.645)? This isolates whether s42's router POLICY alone
# is causally sufficient, vs requiring a backbone that co-adapted with that
...
RUN_OUT="${WORKDIR}/runs/pat225_scale_card/S4_s43_router_from_s42"
ROUTER_SRC="${WORKDIR}/runs/pat225_scale_card/S4_s42/checkpoint-15000"
MASTER_PORT=$(( 24225 + SLURM_JOB_ID % 1000 ))

echo "================= BEGIN RUN PAT-225 S4 s43 (router transplanted from s42, frozen) ================="
...
python -m torch.distributed.run --nproc_per_node=2 --master_port=${MASTER_PORT} ./run_clm.py \
  ...
  --wavelet_router_transplant_from "${ROUTER_SRC}" \
  --wavelet_router_transplant_freeze True \
  --cfg_path "${RUN_OUT}/supply_model.cfg" \
  --seed 43

echo "=== PAT-225 S4 s43 (router from s42, frozen) done ==="
