...
# QWAB medium pretrains right now. Bounced between elm54 (3090x4) and elm66
# (a6000x4) a few times already -- elm66 was grabbed by another user's job the
# moment this script's job was cancelled to fix the detach flag below, so
# settled back on elm54 (3090x4, 24GB/GPU -> bs=8/accum=2, global_bs=64
# unchanged) rather than wait for another idle a6000/6000 node. Restarted from
# step 0 twice already: once for the node move, once for the detach->nodetach
# config fix (14min/~600 steps had run under detach=true on elm66 before that
# fix).
