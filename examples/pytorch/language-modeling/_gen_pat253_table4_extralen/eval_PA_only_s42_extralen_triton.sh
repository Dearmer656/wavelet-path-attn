#!/bin/bash
#SBATCH --job-name=PAonly_hp_triton
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_PAonly_s42_extralen_triton.txt
...
# Table 4 (GPT-2 small) fill-in: PA-only at L8192/12288/16384, triton fallback
# pytorch impl OOM'd even in bf16 on a single 47.4GB a6000 (path_attn itself is memory-heavy at this
# length, independent of wavelet bias — same p6000-only wall medium hit). triton has no wavelet bias
# path anyway, so for a PA-only checkpoint (bias never trained/used) this is numerically equivalent.
...
for BSIZE in 8192 12288 16384; do
  ...
  OUTPUT="${BASE}/hotpot_long/results_uniform/PA_only_s42_uniform_triton/L${BSIZE}"
  mkdir -p "${OUTPUT}"
  echo "=== PA_only s42 L${BSIZE} (triton) ==="
  ...
  python3 -c "import json; d=json.load(open('${OUTPUT}/eval_results.json')); print(f'PA_only s42 L${BSIZE} (triton): F1={d[\"eval_f1\"]:.4f}')"
done

echo "=== Done: PA_only s42 L8192/12288/16384 (triton) ==="
