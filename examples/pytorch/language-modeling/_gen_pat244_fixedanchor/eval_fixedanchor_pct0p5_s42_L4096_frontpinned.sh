#!/bin/bash
#SBATCH --job-name=eval_fixedanchor_pct0p5_L4096_fp
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_fixedanchor_pct0p5_s42_L4096_frontpinned.txt
...
CFG_PATH="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/_gen_pat244_fixedanchor/fixedanchor_pct0p5_true_frontpinned.cfg"
BLOCK_SIZE=4096
[ -d "${CHECKPOINT}" ] || { echo "Missing ${CHECKPOINT}" >&2; exit 1; }
OUTPUT_DIR="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/results_uniform/fixedanchor_pct0p5_s42_ckpt15900_frontpinned/L${BLOCK_SIZE}"
