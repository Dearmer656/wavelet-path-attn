...
# K2 "precision" test: does a minimal K=2 design, using ONLY the two scales that
# K16_s43_optrerun's own router concentrated on (rho=176.9 and rho=337.8, which
# won 26%+21%=47% of all argmax competitions across layers -- far more than the
# other 14 scales combined), match/beat K16's F1=0.7356 with far fewer redundant
# candidate scales? Tests "precise design > wide-span-many-scales" hypothesis.
# me values are the exact fractions matching K16's own scale buffer (me_8=224/15,
# me_9=252/15 in K16's 28*i/15 linspace, i=8,9).
emit_and_submit K2_precision_176_338 2 '[14.933333333333334, 16.8]' 512 128 2 16 2 '#SBATCH --gres=gpu:6000:2 --nodelist=elm73'

echo "=== K2_precision_176_338 submitted ==="
