#!/bin/bash
#SBATCH --job-name=hp2048_medK1_rho256_ricker_s44
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_medK1_rho256_ricker_s44_ckpt15000_hotpot2048.txt
...
CHECKPOINT="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/mix_medium_K1_rho256_ricker_s44/checkpoint-15000"
CFG_PATH="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/mix_medium_K1_rho256_ricker_s44/supply_model.cfg"
BIAS_TYPE=$(grep -oP '(?<=bias_type=")[^"]+' "${CFG_PATH}")
BLOCK_SIZE=2048
[ -d "${CHECKPOINT}" ] || { echo "Missing ${CHECKPOINT}" >&2; exit 1; }
OUTPUT_DIR="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/results_uniform/mix_medium_K1_rho256_ricker_s44_ckpt15000/L${BLOCK_SIZE}"
...
echo "=== Done: mix_medium_K1_rho256_ricker_s44 L${BLOCK_SIZE} ==="
