...
# detach=true (matches the project's standard small-model finetune
# convention, unlike the sibling nodetach run job 579839 on elm72). Needed to
# isolate the per-head-vs-head-shared variable from the detach-vs-nodetach
# variable -- see submit_pat_perhead_router_k1_l512_rho512_6000x4.sh (nodetach
# sibling) and submit_pat_averagehead_router_k1_l512_rho512_nodetach.sh
# (head-shared+nodetach, not actually needed/used) for the full 2x2 grid
# context:
#   head-shared + detach=true:  F1=0.7041 (ckpt5000, existing checkpoint
#     K1_L512_me18_rho512_ricker_s42)
#   per-head    + nodetach:     F1=0.6789 (ckpt5000, job 579839, elm72)
#   per-head    + detach=true:  THIS RUN
#   head-shared + nodetach:     not run (not needed once this exists --
#     detach=true is the convention to match against)
#
# GPU: a6000x2 per explicit request. elm72 (job 579839, the nodetach sibling)
# is left completely untouched -- not stopped/moved, per explicit request, to
# avoid losing that node slot to another user.

set -euo pipefail

WORKDIR="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling"
TAG="perheadrouter_K1_L512_me18_rho512_detach_a6000x2"
...
echo "submitted ${TAG} job=${JID} (a6000x2, unpinned)"
