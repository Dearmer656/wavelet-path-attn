#!/usr/bin/env python3
"""Same as plot_fox_attention_logits_single_case.py, but using a genuinely DISPERSED
evidence example (hotpot_long_dev_uniform.jsonl, which has placement_actual_pct spread
across [0,1], confirmed by direct inspection) and respect_doc_order=True so the
evidence actually lands wherever the source file put it, instead of always being
front-pinned. FoX's finetune and every eval run so far always saw front-pinned
evidence -- this tests whether the "star head" evidence-retrieval circuit found on the
front-pinned case generalizes to a placement the model has never seen, or whether it
was really just a "attend near the front" positional shortcut.

Evidence token span(s) are located by token-subsequence search over the assembled
context (robust regardless of how build_context_budgeted's respect_doc_order=True
reordering scatters the selected sentences), since supporting facts can end up
non-contiguous (different titles can land in different positions) under real
dispersed placement.
"""
import json
import os
...
TARGET_LEN = 4096
BLOCK_SIZE = 4096
PLACEMENT_LO, PLACEMENT_HI = 0.40, 0.60  # pick a genuinely-middle example
OUT_PNG = "/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/results/fox_medium_s42_finetuned_ckpt15000_forgetgate/attention_logits_single_case_L4096_dispersed.png"

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

tokenizer = AutoTokenizer.from_pretrained("gpt2")
run_clm.GLOBAL_TOKENIZER = tokenizer
...
# ---- Find one record with target_total_tokens=4096 and placement roughly in the middle ----
rec = None
with open(JSONL) as f:
    for line in f:
        ...
        if r["meta"]["target_total_tokens"] != TARGET_LEN:
            continue
        pct = r["meta"].get("placement_actual_pct", -1.0)
        if PLACEMENT_LO <= pct <= PLACEMENT_HI:
            rec = r
            break
assert rec is not None, f"no record found with target_total_tokens={TARGET_LEN} and placement in [{PLACEMENT_LO},{PLACEMENT_HI}]"
print(f"Using record {rec['_id']} placement_actual_pct={rec['meta']['placement_actual_pct']:.3f}")
...
# respect_doc_order=True: keep the source file's own (genuinely dispersed) document order
# instead of the default front-pinning.
ctx_text, ctx_ids, status = run_clm.build_context_budgeted(
    ex, budget_context, prefer_same_title=True, min_tokens=0, respect_doc_order=True,
)
assert ctx_ids is not None, f"build_context_budgeted failed: status={status}"

# Locate evidence token span(s) by subsequence search -- robust regardless of where
# respect_doc_order=True scattered them.
...
for t, sid in sf_pairs:
    ...
    n = len(ids)
    found = None
    for start in range(0, len(ctx_ids) - n + 1):
        if ctx_ids[start:start + n] == ids:
            found = start
            break
    if found is not None:
        evidence_spans.append((found, found + n))
    else:
        print(f"WARNING: could not locate evidence span for title={t!r} sid={sid}")

prompt_ids = prefix_ids + ctx_ids + suffix_ids
question_start = len(prefix_ids) + len(ctx_ids)
evidence_spans_abs = [(len(prefix_ids) + a, len(prefix_ids) + b) for a, b in evidence_spans]

print(f"prompt length (context part) = {len(prompt_ids)} tokens, "
      f"evidence spans (absolute) = {evidence_spans_abs}, "
      f"question+answer-prompt = [{question_start}, {len(prompt_ids)})")
...
def attn_row_for_query(layer_idx, head_idx, query_pos):
    ...
    B, T_, _ = q.shape
    q = q.view(B, T_, num_heads, head_dim)[0, :, head_idx, :]
    k = k.view(B, T_, num_heads, head_dim)[0, :, head_idx, :]
    g = f[0, :, head_idx]
    g_cumsum = torch.cumsum(g, dim=0)
    scale = head_dim ** -0.5
    q_i = q[query_pos]
    logits = (k @ q_i) * scale + g_cumsum[query_pos] - g_cumsum
    logits[query_pos + 1:] = float("-inf")
...
for ax, (lid, hid, label) in zip(axes, picks):
    ...
    for a, b in evidence_spans_abs:
        ax.axvspan(a, b, color="#e53e3e", alpha=0.25, label="gold evidence" if (a, b) == evidence_spans_abs[0] else None)
...
fig.suptitle(
    f"FoX medium (finetuned) -- DISPERSED evidence (placement_actual_pct={rec['meta']['placement_actual_pct']:.2f})\n"
    f"HotpotQA-Long L4096, red=gold evidence tokens, green=question/answer-prompt tokens",
    fontsize=10,
)
...
for lid, hid, label in picks:
    probs = attn_row_for_query(lid, hid, query_pos)
    mass_evidence = float(sum(probs[a:b].sum() for a, b in evidence_spans_abs))
