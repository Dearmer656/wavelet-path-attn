#!/bin/bash
# PAT-244: "optimal setting" K1, L_train=512, rho=4096 (me=24), on 2x 6000 (elm73).
# Re-run of the old pre-07-31 checkpoint (S1_me24_s42) using the CURRENT code/cfg
# convention (wavelet_router_norm_mode=rms_joint set explicitly), so no monkey-patch
# is needed for future analysis. with_null (NOT with_null_independent_scales: at K=1
# that stacks a redundant second sigmoid gate on top of g0_gate, which is
# architecturally wrong -- with_null's w=1 collapse is the correct K=1 reduction) +
# wavelet_router_norm_mode=rms_joint + multiscale_norm=rms (inert at K=1, kept for
# consistency with the fixed optimal definition).
set -euo pipefail
...
tag="K1_L512_optimal_me24_rho4096"
K=1
sme=24
block_size=512
chunk_q=128
...
bs=16
accum=2
gres='#SBATCH --gres=gpu:6000:2 --nodelist=elm73'

run_out="${RUN_BASE}/${tag}"
train_sh="${GEN_DIR}/train_${tag}.sh"
test_sh="${GEN_DIR}/test_${tag}.sh"
