#!/bin/bash
#SBATCH --job-name=rotmed_hp_L1024_uniform
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_rotary_medium_s42_L1024_uniform.txt
...
OUTPUT="${BASE}/hotpot_long/results/rotary_medium_s42_ckpt15000/L1024_uniform"
mkdir -p "${OUTPUT}"
cd "${BASE}"

MASTER_PORT=$(( 13000 + SLURM_JOB_ID % 10000 ))

echo "=== Rotary medium (finetuned) s42 HotpotQA-Long L1024_uniform ==="
...
python3 -c "import json; d=json.load(open('${OUTPUT}/eval_results.json')); print(f'Rotary medium (finetuned) s42 L1024_uniform: F1={d[\"eval_f1\"]:.4f}')"

echo "=== Done: Rotary medium (finetuned) s42 HotpotQA-Long L1024_uniform ==="
