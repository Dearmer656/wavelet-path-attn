#! /bin/bash
#SBATCH --job-name=MedMixQWABrho256Detach
#SBATCH --output=log_file/train/%j_mix_medium_qwab_rho256_scratch_detach_10ep.txt
...
# DETACH variant: wavelet_ctx_feat_detach_delta=true (router/shift-proj input
# router/shift-proj sees gradient from the full backbone) -- the existing
# "dd" convention.s own default (hence "dd" = detach delta)
# , paired with the
# nodetach sibling run (train_mix_medium_qwab_rho256_scratch_nodetach_10ep.sh)
...
OUT=runs/mix_medium_owt_qwab_rho256_scratch_detach_10ep
mkdir -p "${OUT}"
...
echo '================= BEGIN RUN (QWAB rho256 scratch backbone, DETACH) ================='
...
echo "=== PAT-258 medium mix QWAB rho256-scratch-backbone DETACH 10ep done ==="
