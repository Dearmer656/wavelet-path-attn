...
# K3 @ L512 with scales chosen RELATIVE to the training length (rho=[128,256,384],
# i.e. 0.25x/0.5x/0.75x of L512), mirroring the L256 design (rho=[64,128,192] =
# 0.25x/0.5x/0.75x of L256) that beat single-scale. Investigation found the
# earlier K3_L512_optimal_center (rho=[2048,4096,6144], centered on K1's best
# single scale, 4-12x the training length) loses to K1 alone because at those
# scales the Ricker wavelets' curvature falls almost entirely OUTSIDE the L512
# training window -- the 3 "different" scales collapse into near-identical,
# near-linear, functionally redundant shapes within the observable context.
emit_and_submit K3_L512_relative_scales 3 '[14, 16, 17.169925001442312]' 512 128 2 16 2 '#SBATCH --gres=gpu:6000:2 --nodelist=elm71'

echo "=== K3_L512_relative_scales submitted ==="
