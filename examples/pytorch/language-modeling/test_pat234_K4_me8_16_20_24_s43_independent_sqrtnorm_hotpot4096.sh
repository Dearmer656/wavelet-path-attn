#!/bin/bash
#SBATCH --job-name=hp4096_K4s43
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_hp4096_K4_me8_16_20_24_s43_independent_sqrtnorm.txt
...
# HotpotQA-Long L4096 (uniform) eval for K4_me8_16_20_24_noC1_s43_independent_sqrtnorm.
# Training (545776) reached checkpoint-15890, but the job's own auto-chained
# eval (via hotpot_long/run_eval_hotpot_long_uniform_a6000x2_cpus4.sh) got cut
# off by cancellation partway through (config.json written, no eval_results.json).
# Resubmitting standalone against checkpoint-15000 (same convention as the
# rest of the K-family, for cross-comparability). No node pinned (a100).
...
RUN_OUT="${LANG_MODEL_DIR}/runs/pat234_scale_card/K4_me8_16_20_24_noC1_s43_independent_sqrtnorm"
CHECKPOINT="${RUN_OUT}/checkpoint-15000"
CFG_PATH="${RUN_OUT}/supply_model.cfg"
MODEL_NAME="K4_me8_16_20_24_independent_sqrtnorm_s43_ckpt15000"
BLOCK_SIZE=4096
