#! /bin/bash
#SBATCH --job-name=PAT162_hln_s44
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/small_hidden_ln_s44/train/%j_small_hidden_ln_s44.txt
...
# PAT-162: QWAB-Hidden-LN ablation — GPT-2 small, s44 (3-seed backfill)
# One-factor delta from Full QWAB small (s42_delta_detach):
#   wavelet_ctx_feat_mode="hidden_ln" (was "q_minus_qcorr_meanh")
# Backbone, data, LR, epochs identical to train_layer_wise_seed42.sh
# Branch: hongyusaatitech/pat-160-... (hidden_ln code: commit f588d66430)
...
RUN_OUT="${WORKDIR}/runs/small_hidden_ln_s44"
MASTER_PORT=$(( 24162 + SLURM_JOB_ID % 1000 ))

echo "================= BEGIN RUN PAT-162 small hidden_ln s44 ================="
...
echo "=== PAT-162 small hidden_ln s44 done ==="
