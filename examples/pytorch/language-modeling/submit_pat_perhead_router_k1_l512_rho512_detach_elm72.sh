#!/bin/bash
# Parallel copy of submit_pat_perhead_router_k1_l512_rho512_detach_a6000x2.sh
# (per-head QWAB router, K1, L512, rho=512, detach=true), pinned to elm72
# (6000x2) per explicit request -- a race against the already-running a6000x2
# copy (job 580322) for whichever GPU pair frees up/finishes first. elm72
# currently has 0 free GPUs (4 held by job 579839, the nodetach sibling which
# stays untouched per explicit request; 1 held by another user), so this will
# queue until 2 free up there specifically.

set -euo pipefail

WORKDIR="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling"
TAG="perheadrouter_K1_L512_me18_rho512_detach_elm72"
...
echo "submitted ${TAG} job=${JID} (elm72, queued, 2 of 5 GPUs)"
