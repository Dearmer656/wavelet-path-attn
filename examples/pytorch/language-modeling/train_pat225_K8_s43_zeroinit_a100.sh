#! /bin/bash
#SBATCH --job-name=PAT225_K8_s43_zeroinit_a100
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/pat225_scale_card/K8_s43_zeroinit/train/%j_pat225_K8_s43_zeroinit.txt
...
# PAT-225: scale-cardinality ablation — GPT-2 small, S=4, s42
# One-factor delta from Full QWAB small (s42_delta_detach):
#   wavelet_ctxscale_k=4 (fixed support [2^0, 2^14], log-uniform)
...
RUN_OUT="${WORKDIR}/runs/pat225_scale_card/K8_s43_zeroinit"
MASTER_PORT=$(( 24225 + SLURM_JOB_ID % 1000 ))

echo "================= BEGIN RUN PAT-225 S=4 s42 ================="
