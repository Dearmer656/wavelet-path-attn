#! /bin/bash
#SBATCH --job-name=PAT226_diagB
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/pat226_mamba2/diag_notf32/train/%j.txt
...
# PAT-226 diagnostic B: 2000-step wt103 pretrain smoke with tf32 disabled
# (keeps the fast/Triton path, isolates TF32 matmul precision loss). If
...
RUN_OUT="${WORKDIR}/runs/pat226_mamba2/diag_notf32"
...
python -m torch.distributed.run --nproc_per_node=4 --master_port=${MASTER_PORT} ./run_clm.py \
  ...
  --warmup_ratio 0.05 --bf16 True --tf32 False \
  --output_dir "${RUN_OUT}" --overwrite_output_dir \
  --seed 42

echo "=== PAT-226 diag B (no tf32) done ==="
