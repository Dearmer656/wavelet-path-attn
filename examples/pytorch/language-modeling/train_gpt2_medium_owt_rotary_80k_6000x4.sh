...
# PAT-164: Plain Rotary (no QWAB) GPT-2 medium OWT pretrain from scratch
# Same recipe as train_gpt2_medium_owt_rotary_qwab_80k_a100x4.sh, but wavelet_router=False
...
cd "${WORKDIR}"
OUT="${WORKDIR}/runs/gpt2_medium_owt_rotary_80k"
mkdir -p "${OUT}/train"

MASTER_PORT=$(( 24300 + SLURM_JOB_ID % 1000 ))

echo "=== Plain Rotary medium OWT pretrain: 4×p6000 ==="
...
echo "=== Plain Rotary medium OWT pretrain done ==="
