#!/bin/bash
#SBATCH --job-name=xsum_mamba2_med
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/mix_medium_owt_mamba2_10ep_s42/ckpt_eval_xsum/%j_xsum_mamba2_medium_mix_10ep.txt
...
# Filtered XSum eval for Mamba2 medium, mix 10ep s42 checkpoint (mirrors
...
# Mamba2 is a recurrent SSM -- no pe_method/attn_implementation flags apply.
# checkpoint-15900 assumed by analogy with the ALiBi/QWAB mix-finetune counterparts (same
# "mix" dataset, 10 epochs, global_bs=64, bs=8/accum2) -- fix CKPT below if the actual
# final checkpoint differs.
# Depends on the mix-finetune (train_pat226_mamba2_medium_mix_10ep_s42.sh) -- submit with
# --dependency=afterok:<finetune_job_id>. Uses mamba2_env (real CUDA causal_conv1d/
# mamba_ssm kernels), matching train/finetune.
...
CKPT="${WORKDIR}/runs/mix_medium_owt_mamba2_10ep_s42/checkpoint-15900"
XSUM_VALIDATION_FILE="/cl/work5/hongyu-s/fact-check-summarization/xsum_test_filter_level2_official_style.jsonl"
RESULT_DIR="${WORKDIR}/runs/mix_medium_owt_mamba2_10ep_s42/ckpt_eval_xsum"
RUN_TAG="${SLURM_JOB_ID:-manual}_$(date -u +%Y%m%dT%H%M%SZ)"
OUT_ROOT="${RESULT_DIR}/xsum_out_${RUN_TAG}"
XSUM_CSV="${RESULT_DIR}/xsum_filter_metrics_mamba2_medium_mix_10ep_${RUN_TAG}.csv"
...
JOB_PORT=$(( 20000 + ${SLURM_JOB_ID:-0} % 1000 ))
...
echo "=== Mamba2 medium mix 10ep s42 XSum eval | ckpt: ${CKPT} ==="
