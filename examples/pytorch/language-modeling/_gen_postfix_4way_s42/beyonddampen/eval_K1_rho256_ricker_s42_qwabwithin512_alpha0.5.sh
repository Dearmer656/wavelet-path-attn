#!/bin/bash
#SBATCH --job-name=postfix_hp2048_K1_rho256_ricker_s42_qwabwithin512a05
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_postfix_K1_rho256_ricker_s42_qwabwithin512a05_ckpt15000_hotpot2048.txt
...
CFG_PATH="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/_gen_postfix_4way_s42/beyonddampen/K1_rho256_ricker_s42_qwabwithin512a05.cfg"
mkdir -p "$(dirname "${CFG_PATH}")"
cp "/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/pat225_postfix_docorder/K1_rho256_ricker_s42/supply_model.cfg" "${CFG_PATH}"
echo 'wavelet_qwab_within_dampen_threshold=512' >> "${CFG_PATH}"
echo 'wavelet_qwab_within_dampen_alpha=0.5' >> "${CFG_PATH}"
BLOCK_SIZE=2048
[ -d "${CHECKPOINT}" ] || { echo "Missing ${CHECKPOINT}" >&2; exit 1; }
OUTPUT_DIR="/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/results_uniform/postfix_K1_rho256_ricker_s42_qwabwithin512a05_ckpt15000/L${BLOCK_SIZE}"
...
echo "=== Done: postfix_K1_rho256_ricker_s42_qwabwithin512a05 L${BLOCK_SIZE} ==="
