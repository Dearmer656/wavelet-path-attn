#! /bin/bash
#SBATCH --job-name=PAT234_K1_me28_C_s42
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/pat234_scale_card/K1_me28_C_s42/train/%j_train.txt
...
# PAT-234: QWAB capacity test — K=1, rho=16384 (max_exp 28), CENTER OFF (variant A).
# Paired atomic test vs variant C (center on). Only delta between A and C is
#   wavelet_logit_bias_center_enable (true here) — set via supply_model.cfg.
# In-training eval DISABLED (--eval_strategy no); wavelet stats kept (logging_steps 500).
...
RUN_OUT="${WORKDIR}/runs/pat234_scale_card/K1_me28_C_s42"
MASTER_PORT=$(( 24234 + SLURM_JOB_ID % 1000 ))

echo "================= BEGIN RUN PAT-234 K1 me28 C (center ON) s42 ================="
...
echo "=== PAT-234 K1 me28 A s42 done ==="
